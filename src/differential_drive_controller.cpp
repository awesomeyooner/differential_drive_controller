...
namespace differential_drive_controller
{
        ...
        std::vector<WheelHandle>& registered_handles)
    {
        ...
        // for each wheel in wheel_names
        for (const std::string& wheel_name : wheel_names)
        {

            // Create the velocity handle
            const hardware_interface::LoanedStateInterface* velocity_handle = nullptr;

            // Create the position handle
            const hardware_interface::LoanedStateInterface* position_handle = nullptr;

            // Create the command handle
            hardware_interface::LoanedCommandInterface* command_handle = nullptr;

            // Search all state interfaces to find the corresponding velocity and position
            // state_interfaces for this current wheel
            for(const hardware_interface::LoanedStateInterface& interface : state_interfaces_)
            {
                // "prefix" is the name of the joint itself 
                bool does_name_match = interface.get_prefix_name() == wheel_name;
                
                // If it's not the correct wheel, then skip
                if(!does_name_match)
                    continue;
                
                // Is this state for velocity?
                bool is_velocity = interface.get_interface_name() == hardware_interface::HW_IF_VELOCITY;

                // Is this state for position?
                bool is_position = interface.get_interface_name() == hardware_interface::HW_IF_POSITION;

                // Assign the appropriate handle based on type
                if(is_velocity)
                    velocity_handle = &interface;
                else if(is_position)
                    position_handle = &interface;
            }

            // Search all state interfaces to find the corresponding command_interface for this current wheel
            for(hardware_interface::LoanedCommandInterface& interface : command_interfaces_)
            {
                ...
                bool does_cmd_type_match = interface.get_interface_name() == params.command_type;

                // If it's the correct command then assign and end the loop
                if(does_cmd_type_match)
                {
                    command_handle = &interface;
                    break;
                }
            }
